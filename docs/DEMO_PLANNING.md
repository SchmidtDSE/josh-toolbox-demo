...
| Parameter | Value |
|-----------|-------|
| Grid dimensions | 20 × 20 patches |
| Patch size | 10m × 10m |
| Total extent | 200m × 200m |
| Total patches | 400 |
| Coordinate system | WGS84 (EPSG:4326) |

### 2.2 Geographic Bounds

Using a small area in the Mojave Desert region for realism:

```
Southwest corner: 35.0°N, -116.002°W
Northeast corner: 35.002°N, -116.0°W
```

Note: The actual geographic location doesn't matter for the toy model, but using realistic coordinates ensures compatibility with Josh's geospatial machinery.
...
2D Gaussian centered on grid, simulating a single ignition point fire:

```python
# Pseudocode
center_x, center_y = 10, 10  # Grid center (0-indexed)
sigma = 5  # Standard deviation in grid cells

for i in range(20):
    for j in range(20):
        distance_sq = (i - center_x)**2 + (j - center_y)**2
        severity[i, j] = 0.95 * exp(-distance_sq / (2 * sigma**2))
```

#### Expected Pattern

- Peak severity: ~0.95 at center
- Moderate severity (0.3-0.7): Ring from ~3-8 cells from center
- Low severity (<0.3): Outer edges
...
| Resolution | ~0.0001° (approximately 10m at this latitude) |
...
start simulation Main
  ...
  grid.size = 10 m
  grid.low = 35.0 degrees latitude, -116.002 degrees longitude
  grid.high = 35.002 degrees latitude, -116.0 degrees longitude
  grid.patch = "Default"
...
# Grid parameters
n_rows, n_cols = 20, 20
lat_low, lat_high = 35.0, 35.002
lon_low, lon_high = -116.002, -116.0
